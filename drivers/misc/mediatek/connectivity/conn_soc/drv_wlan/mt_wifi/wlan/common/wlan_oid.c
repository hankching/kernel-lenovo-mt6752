			...
			IN PVOID pvSetBuffer, IN UINT_32 u4SetBufferLen, OUT PUINT_32 pu4SetInfoLen)
{
	...
	*pu4SetInfoLen = 0;
	...
	if (prAdapter->CfgSetting.ScnStopScan)
	...
	OUT PUINT_32    pu4SetInfoLen)
{
	...
	*pu4SetInfoLen = sizeof(CMD_RLM_INFO_T);
	...
	DBGLOG(INIT, INFO, "<tar_cmd> command = %u %u %u\n",
			...
			prCmdBcnInfo->fgBcnTT_fgKeepAwakeInLastStage);
...
/*----------------------------------------------------------------------------*/
/*!
* \brief This routine is called to query CFG value.
*
* \param[in] pvAdapter Pointer to the Adapter structure.
* \param[out] pvQueryBuf A pointer to the buffer that holds the result of
*                           the query.
...
/*----------------------------------------------------------------------------*/
WLAN_STATUS
wlanoidQueryCfgRead(IN P_ADAPTER_T prAdapter,
		    IN PVOID pvQueryBuffer, IN UINT_32 u4QueryBufferLen, OUT PUINT_32 pu4QueryInfoLen) {
	P_CMD_HEADER_T prCmdV1Header;
	...
	*pu4QueryInfoLen = sizeof(CMD_HEADER_T);

	if (u4QueryBufferLen < sizeof(CMD_HEADER_T))
		return WLAN_STATUS_INVALID_LENGTH;

	pr_debug("wlanoidQueryCfgRead E\n");
	prCmdV1Header = (P_CMD_HEADER_T) pvQueryBuffer;

	/* fill command */
	kalMemSet(&cmdV1Header, 0, sizeof(CMD_HEADER_T));
	kalMemCopy(&cmdV1Header, prCmdV1Header, sizeof(CMD_HEADER_T));
	...
	rStatus = wlanSendSetQueryCmd(
			...
			TRUE,						/* fgNeedResp */
			TRUE,						/* fgIsOid */
			nicCmdEventQueryCfgRead,	/* pfCmdDoneHandler*/
			nicCmdTimeoutCommon,		/* pfCmdTimeoutHandler */
			sizeof(CMD_HEADER_T),		/* u4SetQueryInfoLen */
			(PUINT_8) & cmdV1Header,	/* pucInfoBuffer */
			pvQueryBuffer,				/* pvSetQueryBuffer */
			u4QueryBufferLen			/* u4SetQueryBufferLen */
	);
	pr_debug("wlanoidQueryCfgRead X");
	return rStatus;
}				/* end of wlanoidQueryMcrRead() */
